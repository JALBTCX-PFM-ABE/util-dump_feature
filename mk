...
if [ $SYS = "Linux" ]; then
    DEFS="NVLinux"
    LIBRARIES="-L $PFM_LIB -lnvutility -lBinaryFeatureData -lgdal -lxml2 -lpoppler -lGLU -lm"
    export LD_LIBRARY_PATH=$PFM_LIB:$QTDIR/lib:$LD_LIBRARY_PATH
else
    DEFS="NVWIN3X XML_LIBRARY"
    LIBRARIES="-L $PFM_LIB -lnvutility -lBinaryFeatureData -lgdal -lxml2 -lpoppler -lm -liconv -lstdc++"
